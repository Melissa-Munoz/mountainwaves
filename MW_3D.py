...
def stationary_mountain_wave_3d_rotating_nonhydro(
    ...
    m[evanescent]  = 0#1j * np.sqrt(-m2[evanescent])
    ...
    chi0_hat = np.zeros((nx, ny), dtype=complex)

    eta0_hat[valid] = w0_hat[valid] / (1j * U * k_grid[valid])
    xi0_hat[valid]  = v0_hat[valid] / (1j * U * k_grid[valid])
    chi0_hat[valid] = u0_hat[valid] / (1j * U * k_grid[valid])  # <-- THIS IS χ
    ...
    chi  = np.zeros((nx, ny, nz))  # ADD THIS
    u_hat   = np.zeros((nx, ny, nz),dtype=complex)
    v_hat   = np.zeros((nx, ny, nz),dtype=complex)
    w_hat   = np.zeros((nx, ny, nz),dtype=complex)
    ...
    for iz in range(nz):
        ...
        chi[:, :, iz] = np.fft.ifft2(chi0_hat * vf).real
        u_hat[:, :, iz] = u0_hat * vf
        v_hat[:, :, iz] = v0_hat * vf
        w_hat[:, :, iz] = w0_hat * vf
    ...
    theta_field = np.arctan(np.abs(m))/(np.sqrt(kh2)+eps)
    #plt.contourf(theta_field)
    #plt.show()
    ...
    return {
        ...
        "h": h, "xc": xc, "yc": yc,
        # fields
        "w": w, "u": u, "v": v, "phi": phi, "b": b,  "u_hat": u_hat, "v_hat": v_hat, "w_hat": w_hat,
        # displacements
        "eta": eta, "xi": xi, "chi": chi,
        # spectral
        "m": m, "m2": m2,
        "k": k, "l": l, "k_grid": k_grid, "l_grid":l_grid,
        # params
        "params": dict(
            nx=nx, ny=ny, nz=nz,
            lx=lx, ly=ly, lz=lz,
...
def plot_frequency_line(result):
    import numpy as np
    import matplotlib.pyplot as plt

    p = result["params"]
    U, f, N, ax = p["U"], p["f"], p["N"], p["ax"]
    ...
    xmin = 0.000005
    xmax = 0.1
    #xmin = min(f, omega, N) * 0.2
    #xmax = max(f, omega, N) * 5
    print(xmin,xmax)

    fig, ax = plt.subplots(figsize=(13, 3))

    # horizontal line
    ax.plot([xmin, xmax], [0, 0], color='black', linewidth=1.5)

    # ticks
    ax.plot(f, 0, 'b|', mew=3,ms=20)
    ax.plot(omega, 0, 'k|',mew=3, ms=20)
    ax.plot(N, 0, 'r|', mew=3,ms=20)

    # labels above
    ax.text(f, 0.025, 'f',  fontsize = 26, ha='center',c='b')
    ax.text(omega, 0.025, 'kU', fontsize = 26, ha='center',c='k')
    ax.text(N, 0.025, 'N',  fontsize = 26, ha='center',c='r')
    ...
    # left (evanescent)
    ax.fill_between([xmin, f], -0.16, 0.06, color='lightgray', alpha=0.6)
    ax.text((xmin*8), -0.06, 'Evanescent', fontsize = 16, ha='center')
    ax.text((xmin*8), -0.10, r'$m^2 < 0$', fontsize = 16, ha='center')

    # middle (propagating)
    ax.fill_between([f, N], -0.16, 0.06, color='lightblue', alpha=0.6)
    ax.text(np.sqrt(f*N), -0.06, 'Propagating IG waves', fontsize = 16, ha='center')
    ax.text(np.sqrt(f*N), -0.10, r'$m^2 > 0$', fontsize = 16, ha='center')

    # right (evanescent)
    ax.fill_between([N, xmax], -0.16, 0.06, color='lightgray', alpha=0.6)
    ax.text(xmax*0.31, -0.06, 'Evanescent', fontsize = 16, ha='center')
    ax.text(xmax*0.31, -0.10, r'$m^2 < 0$', fontsize = 16, ha='center')
    ...
    ax.set_xlabel('Frequency (s$^{-1}$)', fontsize = 14)
    #ax.set_title('Wave Regime Diagram')
...
def plot_w_streamlines(result, iz_low=2, n_lines=18):
    ...
    ax.set_title(f"Horizontal cross secction at z={zz_low/1e3:.1f} km",fontsize=18)
    ax.set_xlabel("x (km)")
    ax.set_ylabel("y (km)")
    ax.set_aspect('equal')
    ...
    ax.set_title(r"Vertical cross-section",fontsize=18)
    ax.set_xlabel("x (km)")
    ax.set_ylabel("z (km)")
    ...
    # ==================================================
    # MIDDLE: Cross-stream (y–z)
    # ==================================================
    ax = axes[2]

    ix = len(x) // 2   # center slice in x

    w_yz = w[ix, :, :]

    vmax = np.nanpercentile(np.abs(w_yz), 98) or 1
    ...
    # streamlines using χ (correct variable)
    Y_stream = result['Y'][ix, :][np.newaxis, :] + chi[ix, :, :]
    levels_y = np.linspace(y[0], y[-1], n_lines)

    ax.contour(
        y/1e3, z/1e3, Y_stream.T,
        levels=levels_y,
        colors='k',
        linewidths=1.2
    )
    ...
    # mountain slice
    h_slice = h[ix, :]
    ax.fill_between(y/1e3, 0, h_slice/1e3, color='0.5')

    ax.set_xlim(y[0]/1e3, y[-1]/1e3)
    ax.set_ylim(0, z[-1]/1e3)

    ax.set_title("Transverse cross-section", fontsize=18)
    ax.set_xlabel("y (km)")
    ax.set_ylabel("z (km)")
    ax.set_aspect('auto')
...
def plot_phi_streamlines(result, iz_low=2, n_lines=18):
    ...
    chi     = result['chi'] 
    h       = result['h']
    k_grid, l_grid = result['k_grid'], result['l_grid']
    p = result['params']
    U,f,N = p['U'],p['f'],p['N']
    ...
    w_hat_z       = result['w_hat']
    amp = np.mean(np.abs(w_hat_z), axis=2)

    # remove k=0
    amp[np.abs(k_grid) < 1e-12] = 0.0

    i_dom, j_dom = np.unravel_index(np.argmax(amp), amp.shape)

    k_dom = k_grid[i_dom, j_dom]
    l_dom = l_grid[i_dom, j_dom]
    m_dom2 = (k_dom**2 + l_dom**2) * (N**2 - U**2*k_dom**2) / (U**2*k_dom*2 - f**2)
    m_dom = np.sign(k_dom)*np.sqrt(m_dom2)
    slope = - k_dom/m_dom
    print(x)
    z_tilt = slope * (x - x[-1]/2)
    print(slope)
    ...
    # streamlines (y + ξ)
    Y_stream = y[np.newaxis, :] + xi[:, :, iz_low]
    #Y_stream = result['Y'] + result['chi'][:, :, iz_low]
    levels_y = np.linspace(y[0], y[-1], n_lines)
    ...
    ax.set_title(f"Horizontal cross-section at z={zz_low/1e3:.1f} km",fontsize=18)
    ax.set_xlabel("x (km)")
    ax.set_ylabel("y (km)")
    ax.set_aspect('equal')
    ...
    ax.set_title("Vertical cross-section",fontsize=18)
    ax.set_xlabel("x (km)")
    ax.set_ylabel("z (km)")
    ...
    vmax = np.nanpercentile(np.abs(phi_yz), 98) or 1

    cf = ax.contourf(
        y/1e3, z/1e3, phi_yz.T,
        levels=40,
        cmap='RdBu_r',
        vmin=-vmax, vmax=vmax
    )
